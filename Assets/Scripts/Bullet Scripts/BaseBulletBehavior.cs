    ...
    private void Update()
    {
        ...
        if (Physics.Raycast(ray, out rayHit, rayDistance, LayerMask.GetMask("PlayerBulletTarget")))
        {
            ...
            if (hitComponentHealth != null)
            {
                ...
                // Ask Level manager to spawn a blood particle at the location of the hit enemy
                LevelManager.Instance.SpawnBloodParticleAtLocation(rayHit.transform.position);
